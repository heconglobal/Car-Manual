...
export function createViewer(container, onSelect) {
 ...
 const builders={engine:createEngineDetail,transmission:createTransmissionDetail,'cooling-system':createCoolingDetail};
 ...
 function frameAssembly(name){
  ...
  const direction=name?new T.Vector3(...(name==='home'&&familyFor(current.assembly).id==='cooling-system'?[-.95,.6,1.5]:name==='home'&&current.assembly==='water-pump-detail'?[.9,.65,1.15]:name==='home'&&['coil-detail','distributor-detail'].includes(current.assembly)?[-.9,.8,1.6]:presets[name]||presets.home)).normalize():camera.position.clone().sub(controls.target).normalize();
