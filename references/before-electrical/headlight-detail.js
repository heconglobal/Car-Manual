...
export function buildVehicleHeadlights(groups,materials){const detail=new Map(headlightParts.map(p=>[p.id,new T.Group()])),h=geometryTools(detail,headlightMaterials(materials));buildHeadlights(h,detail);h.optimize();for(const g of detail.values())for(const mesh of [...g.children]){mesh.userData.partId='headlights';groups.get('headlights').add(mesh);}}
...
export function buildHeadlights(h,groups){for(const [side,s]of [['left',1],['right',-1]]){buildLamp(h,side,s);buildDoor(h,groups,side,s);buildMotor(h,side,s);duplicateClosedBucket(groups,side);}buildRelays(h);}
