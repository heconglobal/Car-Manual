...
const state={system:'all',selected:null,tab:'component',query:'',hideBody:false,isolate:false,explode:0,labels:false,wireframe:false,tour:null,step:0,camera:'home',configuration};
...
document.querySelector('#app').innerHTML=`
 ...
 <main class="workspace">
  <aside class="sidebar" aria-label="Systems and components">
   ...
   <p class="section-label">SYSTEMS <span>09</span></p><nav id="systems" aria-label="Vehicle systems"></nav>
...
function getPart(){return parts.find(p=>p.id===state.selected);}
function systemName(id){return systems.find(s=>s.id===id)?.name||'Whole vehicle';}
function renderSystems(){document.querySelector('#systems').innerHTML=systems.map(s=>`<button class="system-button ${state.system===s.id?'active':''}" data-system="${s.id}" aria-pressed="${state.system===s.id}">${icon(s.icon)}<span>${s.name}</span><small>${s.id==='all'?parts.length:parts.filter(p=>p.system===s.id).length}</small></button>`).join('');}
function renderParts(){
 const q=state.query.trim().toLowerCase();const filtered=parts.filter(p=>q?`${p.name} ${p.description} ${p.location} ${systemName(p.system)}`.toLowerCase().includes(q):state.system==='all'||p.system===state.system);
 ...
 document.querySelector('#part-list').innerHTML=filtered.length?filtered.map(p=>`<button class="part-button ${state.selected===p.id?'active':''}" data-part="${p.id}" aria-pressed="${state.selected===p.id}"><span class="part-dot" style="--part-color:${systems.find(s=>s.id===p.system).color}"></span><span>${p.name}</span>${icon('chevron')}</button>`).join(''):`<div class="empty-state">No components match “${esc(state.query)}”.<br/><button class="text-button" data-action="clear-search">Clear search</button></div>`;
}
...
function renderInspector(){
 ...
 if(state.tab==='component')panel.innerHTML=part?`
  ...
  <p class="section-label">START EXPLORING</p>${tours.slice(0,2).map(t=>`<button class="guide-link" data-tour="${t.id}">${icon('play')}<span>${t.title}<small>${t.subtitle}</small></span>${icon('arrow')}</button>`).join('')}
...
function sync(){
 ...
 document.querySelector('#explode').value=Math.round(state.explode*100);document.querySelector('#explode-value').textContent=`${Math.round(state.explode*100)}%`;
...
function selectPart(id){const part=parts.find(p=>p.id===id);if(!part)return;state.selected=id;state.system=part.system;state.tab='component';state.tour=null;if(part.system==='body')state.hideBody=false;sync();viewer?.focus(id);document.querySelector('.sidebar').classList.remove('mobile-open');}
function applyStep(index){const tour=tours.find(t=>t.id===state.tour);if(!tour)return;state.step=Math.max(0,Math.min(index,tour.steps.length-1));const step=tour.steps[state.step];state.system=step.system;state.selected=step.part||null;state.isolate=false;state.hideBody=false;state.explode=step.explode||0;state.camera=step.camera;state.tab='tours';sync();viewer?.view(step.camera);if(tour.id==='air-cleaner')setTimeout(()=>{if(state.tour===tour.id&&state.step===index)viewer?.focus(step.part);},180);}
function reset(){Object.assign(state,{system:'all',selected:null,query:'',hideBody:false,isolate:false,explode:0,wireframe:false,tour:null,step:0,camera:'home'});document.querySelector('#search').value='';sync();viewer?.view('home');}
function openModal(type){
 const modal=document.querySelector('#modal');document.querySelector('#modal-content').innerHTML=`<div class="modal-heading"><p class="eyebrow">WORKSHOP / 85</p><button class="icon-button" data-action="close-modal" aria-label="Close dialog">${icon('close')}</button></div>${type==='sources'?`<h2>Reference library</h2><p class="description">Source coverage is explicit. A source listing does not mean all of its content has been verified or imported.</p>${sources.map(s=>`<article class="source-card"><h3>${s.title}</h3><p>${s.note}</p>${s.url?`<a href="${s.url}" target="_blank" rel="noopener noreferrer">Open source ↗</a>`:''}</article>`).join('')}`:`<h2>A 3D workshop in progress.</h2><p class="description">Build 0.2 rebuilds the vehicle surfaces and materials around factory references: ${parts.length} selectable assemblies, nine systems, three guided tours, exploded views and local UAT feedback.</p><div class="source-card"><h3>What this build represents</h3><p>The vehicle is an owner-described original 1985 Fiero SE 2M6, four-speed manual, with WS6. VIN identity was checked with NHTSA.</p></div><div class="source-card"><h3>What is still in development</h3><p>The procedural model is illustrative, not a measured replica. Individual fasteners, internal engine and gearbox mechanisms, exact routing, full accessory options and validated repair instructions are not complete. Technical service values are withheld until checked against factory references.</p></div><div class="source-card"><h3>Your data</h3><p>Feedback and checklist results are stored locally in this browser. Export them from the UAT tab to share or back up. No analytics, accounts or external assets are loaded by the app.</p></div>`}`;modal.showModal();
}
document.addEventListener('click',async e=>{
 const b=e.target.closest('button');if(!b)return;
 if(b.dataset.paint){state.configuration.paint=b.dataset.paint;saveConfiguration();return;}
 if(b.dataset.system){state.system=b.dataset.system;state.selected=null;state.isolate=false;state.tour=null;state.query='';document.querySelector('#search').value='';sync();viewer?.view('home');return;}
 ...
 switch(b.dataset.action){
  ...
  case 'show-exterior':state.system='all';state.selected=null;state.hideBody=false;state.isolate=false;state.explode=0;state.camera='home';sync();viewer?.view('home');break;
  case 'show-cabin':state.system='interior';state.selected=null;state.hideBody=true;state.isolate=false;state.camera='home';sync();viewer?.view('home');break;
  case 'show-chassis':state.system='all';state.selected=null;state.hideBody=true;state.isolate=false;state.camera='home';sync();viewer?.view('home');break;
  case 'reset':reset();toast('Vehicle view reset');break;
