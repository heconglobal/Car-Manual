...
export function createViewer(container, onSelect) {
 ...
 function update(next) {
  ...
  const dark=current.configuration?.studio==='dark';scene.background.set(dark?'#22282c':'#dededb');scene.fog.color.copy(scene.background);floor.material.color.set(dark?'#292e32':'#dededb');dimensions.visible=!!current.configuration?.dimensions;grid.visible=dimensions.visible;
 ...
 function focus(id) {
  const g=groups.get(id);if(!g)return;root.updateMatrixWorld(true);const bounds=visibleBounds(g);if(bounds.isEmpty())return;const centre=bounds.getCenter(new T.Vector3());const size=bounds.getSize(new T.Vector3()).length();
 ...
 function animate(){if(disposed)return;frame=requestAnimationFrame(animate);const now=performance.now();const dt=Math.min((now-lastTime)/1000,.5);lastTime=now;const cameraBlend=reducedMotion?1:1-Math.exp(-7*dt);const partBlend=reducedMotion?1:1-Math.exp(-10*dt);
  ...
  for(const g of groups.values()){const goal=g.userData.spread.clone().multiplyScalar(current.explode);if(g.position.distanceToSquared(goal)>.000001){g.position.lerp(goal,partBlend);moving=true;renderer.shadowMap.needsUpdate=true;}else g.position.copy(goal);}
  ...
  if(current.selected&&groups.get(current.selected).visible){visibleBounds(groups.get(current.selected),highlight.box);highlight.visible=!highlight.box.isEmpty();}else highlight.visible=false;
  ...
  occlusion.enabled=current.system==='all'&&!current.isolate&&!current.hideBody&&!current.wireframe&&current.explode<.01;
 ...
 function getPartBounds(id){const g=groups.get(id);if(!g)return null;root.updateMatrixWorld(true);const b=visibleBounds(g);return b.isEmpty()?null:{min:b.min.toArray(),max:b.max.toArray()};}
 return {update,view,focus,resize,getState:()=>({...current}),getPartCount:()=>groups.size,getPartBounds,getModelStats:()=>{let meshes=0,triangles=0;root.traverseVisible(o=>{if(o.isMesh){meshes++;triangles+=(o.geometry.index?.count||o.geometry.attributes.position.count)/3;}});return{meshes,triangles};},dispose(){disposed=true;cancelAnimationFrame(frame);observer.disconnect();controls.dispose();scene.traverse(o=>{o.geometry?.dispose();if(o.material)for(const m of [o.material].flat())m.dispose();});occlusion.dispose();output.dispose();composer.dispose();envMap.dispose();pmrem.dispose();renderer.dispose();}};
}
