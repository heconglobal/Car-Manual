...
export function buildOilPan(h){
 ...
 for(const x of [-.172,-.115,-.054,.01,.074,.14,.177])for(const z of [-.121,.121]){
  h.cyl(id,.006,.002,[x,.942,z],'zinc',[0,0,0]);h.bolt(id,[x,.945,z],.0042,'y','zinc');
 }
 ...
 h.cyl(id,.011,.006,[.132,.845,-.104],'blackPaint',[Math.PI/2,0,0]);h.bolt(id,[.132,.845,-.109],.007,'z','zinc');
}
