...
function engineInspector(part){
 ...
 <details class="engine-coverage"><summary>Modeled components & reference limits</summary><p>${engineParts.length} selectable parts and grouped sets across ${engineSections.length-1} subassemblies. Includes rotating assembly, valve gear, intake, timing, lubrication, accessories and ${engineMembers('ignition').length} ignition parts / sets. The distributor opens into its own assembly, including the ICM and pickup.</p><p>Fasteners and some seals are grouped. Full harness routing, accessory internals and some catalog items remain missing. Shapes use GM engine and L44 distributor illustrations plus the 1985 EST diagram; this is not a complete parts inventory or a repair sequence.</p><button class="text-button" data-action="sources">View vehicle-wide parts coverage</button></details>`;
}
