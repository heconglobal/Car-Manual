...
const legacyCoolantRoutes={
 ...
 'front-inlet':[[.385,.588,-1.68],[.52,.56,-1.58],[.60,.40,-1.43],[.64,.235,-1.25]],
 'front-outlet':[[-.385,.305,-1.68],[-.49,.30,-1.55],[-.60,.27,-1.43],[-.64,.235,-1.25]],
...
export function buildCooling(h){
 ...
 annulus(id('tank-right'),.024,.019,.032,[-.382,.659,-1.719],'plastic',[0,0,Math.PI/2]);tube(id('tank-right'),[[-.36,.653,-1.713],[-.338,.654,-1.70]],.004,'plastic');
 cyl(id('pressure-cap'),.030,.009,[-.382,.681,-1.719],'zinc',[0,0,0]);box(id('pressure-cap'),[.075,.007,.018],[-.382,.683,-1.719],'zinc',[],{},.003);annulus(id('pressure-cap'),.020,.008,.003,[-.382,.658,-1.719],'rubber',[0,0,Math.PI/2]);spring(id('pressure-cap'),[-.382,.666,-1.719],.009,.017,'y',5,.001,'zinc');
 cyl(id('drain'),.008,.017,[-.409,.264,-1.675],'plastic',[0,0,0]);box(id('drain'),[.025,.005,.008],[-.409,.254,-1.675],'plastic',[],{},.002);
 ...
 tube(id('recovery-hose'),[[-.338,.654,-1.70],[-.32,.605,-1.62],[-.395,.564,-1.51],[-.465,.574,-1.48]],.004,'rubber');for(const x of [-.55,-.39]){box(id('recovery-bracket'),[.031,.090,.008],[x,.48,-1.551],'zinc',[-.1,0,0],{},.004);bolt(id('recovery-bracket'),[x,.509,-1.546],.004,'z');}
 ...
 // Heater core preview; retain the early separate return to the engine.
 for(let j=0;j<23;j++){
  box(id('heater-core'),[.175,.003,.043],[-.39,.51+j*.0068,-.54],'copper',[],{},.001);
  if(j<22)surface(id('heater-core'),120,2,(u,v)=>[-.4775+u*.175,.513+j*.0068+Math.sin(u*Math.PI*120)*.0027,-.562+v*.044],'copper');
 }
 for(const s of [-1,1]){box(id('heater-tanks'),[.025,.172,.048],[-.39+s*.102,.585,-.54],'copper',[],{},.010);box(id('heater-seals'),[.027,.177,.006],[-.39+s*.102,.585,-.51],'rubber');}
 for(const [i,x] of [[0,-.49],[1,-.29]])tube(id('heater-tanks'),[[x,.634,-.54],[x,.65,-.59],[x,.63,-.65]],.009,'copper');
 for(const [i,x] of [[0,-.14],[1,-.18]]){
  tube(id('heater-pipes'),[[x,.31,-.56],[x,.215,-.36],[x,.215,.54],[x-.04,.35,.73]],i?.008:.009,'blue');
  tube(id('heater-hoses'),[[i?-.29:-.49,.63,-.65],[i?-.28:-.43,.44,-.65],[x,.31,-.56]],.011,'rubber');
 }
