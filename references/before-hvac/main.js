...
import {detailParts,detailPartById,detailSections,detailSectionById,detailMembers,familyFor,detailFamilies} from './inspection-catalog.js';
...
function engineInspector(part){
 const section=detailSectionById.get(state.assembly),family=familyFor(state.assembly),children=detailSections.filter(s=>s.parent===state.assembly);
 ...
 ${children.length?`<p class="section-label">OPEN A SUBASSEMBLY</p><div class="engine-section-list">${children.map(s=>`<button class="guide-link" data-assembly="${s.id}"><span>${s.name}<small>${detailMembers(s.id).length} selectable parts / sets</small></span>${icon('chevron')}</button>`).join('')}</div>`:`<button class="text-button reference-link" data-assembly="${family.id}">← ${detailSectionById.get(family.id).name}</button>`}
