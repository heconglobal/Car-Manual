...
export function buildVehiclePowertrain(groups,materials){
 ...
 for(const p of coolingParts){const target=p.section==='cool-radiator'||p.id==='cool-pressure-cap'?'radiator':p.section==='cool-recovery'?'coolant-reservoir':'coolant-pipes';for(const mesh of [...cool.get(p.id).children]){mesh.userData.partId=target;groups.get(target).add(mesh);}}
}
