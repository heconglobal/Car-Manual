...
function renderSystems(){if(state.assembly){document.querySelector('#systems').innerHTML=engineSections.map(s=>`<button class="system-button ${state.assembly===s.id?'active':''}" data-assembly="${s.id}" aria-pressed="${state.assembly===s.id}">${icon('engine')}<span>${s.name}</span><small>${engineMembers(s.id).length}</small></button>`).join('');return;}document.querySelector('#systems').innerHTML=systems.map(s=>`<button class="system-button ${state.system===s.id?'active':''}" data-system="${s.id}" aria-pressed="${state.system===s.id}">${icon(s.icon)}<span>${s.name}</span><small>${s.id==='all'?parts.length:parts.filter(p=>p.system===s.id).length}</small></button>`).join('');}
function renderParts(){
 const q=state.query.trim().toLowerCase();const filtered=state.assembly?engineMembers(state.assembly).filter(p=>!q||`${p.name} ${p.description}`.toLowerCase().includes(q)):parts.filter(p=>q?`${p.name} ${p.description} ${p.location} ${systemName(p.system)}`.toLowerCase().includes(q):state.system==='all'||p.system===state.system);
...
function renderInspector(){
 ...
 if(state.tab==='component')panel.innerHTML=part?`
  ...
  ${part.system==='engine'?engineEntry():''}
...
function engineEntry(){return `<button class="primary full-width engine-entry" data-assembly="engine">${icon('layers')} Explore engine components</button>`;}
function engineInspector(part){
 const section=engineSectionById.get(state.assembly);
 return `<nav class="assembly-breadcrumb" aria-label="Assembly path"><button class="text-button" data-action="exit-assembly">Vehicle</button><span>/</span><button class="text-button" data-assembly="engine">2.8L V6</button>${state.assembly!=='engine'?`<span>/</span><span>${section.name}</span>`:''}</nav>
 ...
 ${part?`<div class="detail-actions"><button class="secondary" data-action="focus">Focus part</button><button class="secondary" data-action="isolate" aria-pressed="${state.isolate}">${state.isolate?'Show context':'Isolate'}</button></div><p class="description small">${part.source} · <a href="${engineSource}#page=${part.source.endsWith('H-19')?14:17}" target="_blank" rel="noopener noreferrer">Factory illustration ↗</a></p>${state.assembly==='engine'?`<button class="secondary full-width" data-assembly="${part.section}">Open ${engineSectionById.get(part.section).name}</button>`:''}<button class="text-button reference-link" data-action="note-part">${icon('note')} Add a UAT note about this part</button>`:''}
 ${state.assembly==='engine'?`<p class="section-label">OPEN A SUBASSEMBLY</p><div class="engine-section-list">${engineSections.slice(1).map(s=>`<button class="guide-link" data-assembly="${s.id}"><span>${s.name}<small>${engineMembers(s.id).length} selectable parts / sets</small></span>${icon('chevron')}</button>`).join('')}</div>`:`<button class="text-button reference-link" data-assembly="engine">← Complete engine</button>`}
 <details class="engine-coverage"><summary>Modeled components & reference limits</summary><p>${engineParts.length} selectable parts and grouped sets across ${engineSections.length-1} subassemblies. Includes six pistons and rods, twelve valves, springs, rockers, pushrods and lifters, plus intake, timing, lubrication and engine-mounted accessories.</p><p>Fasteners and some seals are grouped. Small clips, full wiring, accessory internals and every catalog item are not yet modeled. Shapes and separation paths are reconstructed from GM 22P H-19 / H-22; this is not a complete parts inventory or a repair sequence.</p></details>`;
}
function openAssembly(id){
 ...
 document.querySelector('#search').value='';sync();viewer?.frameAssembly('home');document.querySelector('.sidebar').classList.remove('mobile-open');
}
