...
export function buildMechanics(h){
 ...
 for(const z of [.99,1.34]){
  ...
  for(let i=0;i<3;i++){
   ...
   tube('engine-block',[[x,.69,z],[x,.71,z<1.1?.91:1.42],[x+.01,.81,1.33],[-.12,.82,1.37]],.005,'rubber');
  }
 }
 ...
 cyl('clutch',.173,.035,[-.17,.495,1.17],'metal');
 cyl('clutch',.146,.017,[-.198,.495,1.17],'dark');
 cyl('clutch',.11,.014,[-.212,.495,1.17],'rotor');
 for(let i=0;i<12;i++){const a=i/12*Math.PI*2;bolt('clutch',[-.216,.495+Math.sin(a)*.144,1.17+Math.cos(a)*.144],.006,'x');}
 cyl('gearbox',.197,.10,[-.237,.50,1.17],'metal',[0,0,Math.PI/2],.167);
 box('gearbox',[.275,.245,.335],[-.40,.47,1.18],'metal',[],{},.06);
 cyl('gearbox',.125,.275,[-.40,.357,1.22],'metal');
 box('gearbox',[.28,.13,.22],[-.41,.604,1.15],'metal',[],{},.03);
 for(let i=0;i<7;i++)box('gearbox',[.013,.22,.31],[-.53+i*.04,.48,1.18],'metal',[],{},.004);
 for(let i=0;i<12;i++){const a=i/12*Math.PI*2;bolt('gearbox',[-.297,.50+Math.sin(a)*.184,1.17+Math.cos(a)*.184],.006,'x');}
 for(const z of [1.035,1.306])for(const x of [-.52,-.36])bolt('gearbox',[x,.611,z],.006);
 label('gearbox','4 SPEED',[.105,.035],[-.546,.53,1.14],[0,-Math.PI/2,0],{background:'#848989',foreground:'#363b3d',font:'bold 53px Arial'});
 cyl('axles',.022,1.36,[0,.307,1.1865],'metal');
 for(const s of [-1,1])for(const x of [.57,.25]){
  cyl('axles',.047,.10,[s*x,.307,1.1865],'rubber');for(let i=0;i<6;i++)ring('axles',.046-i*.002,.006,[s*(x-.04+i*.015),.307,1.1865],'rubber');
 }
 ...
 for(const s of [-1,1])for(const z of [-1.1865,1.1865]){
  ...
  const rotor=z<0?'front-rotors':'rear-rotors';cyl(rotor,.1234,.0108,[x-s*.031,y,z],'rotor');cyl(rotor,.048,.033,[x-s*.03,y,z],'metal');
  ring(rotor,.112,.001,[x-s*.04,y,z],'rotor');ring(rotor,.093,.001,[x-s*.04,y,z],'rotor');
  box('calipers',[.085,.12,.070],[x-s*.087,y+.009,z+.101],'metal',[],{},.022);box('calipers',[.040,.08,.071],[x-s*.035,y+.009,z+.101],'dark');
  for(const dy of [-.041,.041])bolt('calipers',[x-s*.133,y+dy,z+.106],.006,'x');
 }
 // Hydraulic booster / master cylinder.
 cyl('master-cylinder',.105,.075,[.43,.589,-.67],'blackPaint',[Math.PI/2,0,0]);
 cyl('master-cylinder',.026,.20,[.43,.605,-.79],'metal',[Math.PI/2,0,0]);box('master-cylinder',[.087,.057,.146],[.43,.65,-.79],'white');box('master-cylinder',[.094,.014,.151],[.43,.685,-.79],'dark');
 for(const s of [-1,1])tube('master-cylinder',[[.43+s*.02,.594,-.83],[.43+s*.04,.49,-.90],[s*.53,.38,-1.10]],.003,'metal');
 ...
 box('coolant-reservoir',[.225,.205,.17],[-.37,.443,-1.535],'reservoir',[-.1,0,0],{},.045);
 cyl('coolant-reservoir',.025,.028,[-.37,.556,-1.535],'plastic',[0,0,0]);
 tube('coolant-reservoir',[[-.37,.563,-1.535],[-.40,.58,-1.62],[-.36,.605,-1.70]],.005,'rubber');
 ...
 // Radiator and real-looking hoses, clamps and coolant pipes.
 box('radiator',[.70,.35,.056],[0,.452,-1.70],'dark',[-.13,0,0]);
 for(let i=0;i<46;i++)box('radiator',[.007,.32,.059],[-.332+i*.0147,.452,-1.70],'metal',[-.13,0,0],{},.001);
 for(const x of [-.386,.386])box('radiator',[.08,.38,.084],[x,.452,-1.70],'plastic',[-.13,0,0],{},.022);
 ring('radiator',.183,.020,[0,.452,-1.621],'plastic',[0,0,0]);
 cyl('radiator',.052,.06,[0,.452,-1.608],'dark',[Math.PI/2,0,0]);
 for(let i=0;i<5;i++){const a=i/5*Math.PI*2;box('radiator',[.058,.147,.012],[Math.sin(a)*.104,.452+Math.cos(a)*.104,-1.609],'plastic',[0,.15,a],{},.023);}
 for(const s of [-1,1]){
  tube('coolant-pipes',[[s*.66,.221,-1.30],[s*.66,.201,-.6],[s*.66,.201,.51],[s*.56,.27,.80]],.017,'blue');
  tube('coolant-pipes',[[s*.36,.53,-1.72],[s*.58,.45,-1.53],[s*.66,.221,-1.30]],.023,'rubber');
  tube('coolant-pipes',[[s*.56,.27,.80],[s*.52,.42,.91],[s*.38,.62,1.05]],.023,'rubber');
  for(const z of [-.5,.30])box('coolant-pipes',[.046,.008,.028],[s*.66,.18,z],'alloy');
 }
 cyl('thermostat',.031,.09,[.35,.749,1.33],'metal',[0,0,0]);cyl('thermostat',.045,.013,[.35,.802,1.33],'alloy',[0,0,0]);box('thermostat',[.099,.014,.025],[.35,.808,1.33],'alloy');
...
export function buildInterior(h){
 ...
 gauge(.247,.825,-.345,'MPH',85);gauge(.427,.825,-.345,'RPM',6);
 ...
 for(const variant of ['am','amfm','cassette','equalizer']){
  const flags={option:'radio',value:variant};box('dashboard',[.153,.068,.013],[0,.602,-.373],'dark',[],flags);
  label('dashboard',variant==='am'?'AM  850':'FM  98.5',[.076,.021],[0,.615,-.363],[0,0,0],{background:'#112027',foreground:'#aaa994',font:'48px monospace'},flags);
  for(const s of [-1,1])cyl('dashboard',.011,.011,[s*.061,.585,-.362],'plastic',[Math.PI/2,0,0],.011,flags);
  if(['cassette','equalizer'].includes(variant))box('dashboard',[.081,.008,.007],[0,.582,-.361],'plastic',[],flags);
  if(variant==='equalizer')for(let i=0;i<5;i++)box('dashboard',[.004,.021,.004],[-.034+i*.017,.561,-.36],'alloy',[],flags);
 }
 box('dashboard',[.137,.038,.008],[0,.509,-.365],'dark');
 label('dashboard','COLD   ━━━━━   HOT',[.12,.014],[0,.519,-.359],[0,0,0],{font:'42px Arial',foreground:'#b6bdc0'});
 box('dashboard',[.013,.012,.006],[.027,.502,-.355],'alloy');
 ...
 tube('steering-wheel',[[.30,.70,-.26],[.21,.71,-.26]],.006,'dark');
 box('steering-wheel',[.043,.013,.017],[.205,.71,-.26],'plastic',[],{option:'cruise',value:true});
 ...
 label('shifter','1 3 R',[.025,.012],[0,.603,-.177],[-Math.PI/2,0,0],{width:128,height:64,font:'46px Arial',foreground:'#cecfc9'});
 ...
 for(const x of [.226,.34,.46]){tube('pedals',[[x,.52,-.56],[x,.30,-.47]],.01,'metal');box('pedals',[.052,.068,.018],[x,.282,-.457],'rubber',[-.4,0,0],{},.008);for(let i=0;i<5;i++)box('pedals',[.046,.002,.003],[x,.262+i*.010,-.445],'dark',[-.4,0,0],{},.0006);}
