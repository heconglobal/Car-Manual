...
export function createVehicle() {
 ...
 for(const [id,g] of groups)if(g.userData.system==='engine'||['gearbox','clutch','alternator','thermostat'].includes(id))g.traverse(m=>{if(m.isMesh){m.updateMatrix();m.geometry.applyMatrix4(m.matrix);m.geometry.applyMatrix4(engineEnvelope);m.position.set(0,0,0);m.rotation.set(0,0,0);m.scale.set(1,1,1);}});
 // Vehicle +X is the driver's side. The gearbox belongs on that side;
 // the accessory-drive end of the V6 is on the passenger side.
 for(const id of ['engine-block','heads','intake','oil-pan','gearbox','clutch','alternator','thermostat']){
