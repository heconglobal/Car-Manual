...
import {engineParts,enginePartById} from './engine-catalog.js';
...
export function createViewer(container, onSelect) {
 ...
 let engine=null;
 const currentGroups=()=>current.assembly?engine.groups:groups;
 const currentParts=()=>current.assembly?engineParts:parts;
 const currentRoot=()=>current.assembly?engine.root:root;
 ...
 const presets={home:[4.9,2.35,-6.1],front:[.1,1.30,-7.1],rear:[4.6,2.25,5.8],side:[7.6,1.35,0],top:[0,7.8,.001]};
 ...
 for(const part of [...parts,...engineParts]) {
 ...
 function update(next) {
  ...
  if(next.assembly&&!engine){engine=createEngineDetail();scene.add(engine.root);}
  current={...current,...next};
  root.visible=!current.assembly;if(engine)engine.root.visible=!!current.assembly;
  ...
  for(const [id,g] of currentGroups()) {
   ...
   g.visible=current.assembly?(current.assembly==='engine'||g.userData.section===current.assembly):!(current.hideBody&&g.userData.system==='body'&&id!=='spaceframe');
   ...
   g.traverse(o=>{if(!o.isMesh)return;const original=o.userData.original;const ghost=!active&&!current.isolate;
    ...
    if(current.wireframe){o.userData.wireMaterial??=new T.MeshBasicMaterial({wireframe:true,side:T.DoubleSide});o.material=o.userData.wireMaterial;}else o.material=o.userData.surfaceMaterial;
  ...
  renderer.domElement.dataset.assembly=current.assembly||'';
 }
 function view(name) {if(current.assembly){frameAssembly(name);return;}cameraGoal=new T.Vector3(...(presets[name]||presets.home));targetGoal=new T.Vector3(0,.58,0);}
 function engineGoal(g){const goal=g.userData.spread.clone();if(current.assembly==='engine')goal.add(g.userData.assemblySpread);return goal.multiplyScalar(current.assemblyExplode||0);}
 function frameAssembly(name){
  if(!current.assembly||!engine)return;
  const bounds=new T.Box3();engine.root.updateMatrixWorld(true);
  for(const g of engine.groups.values())if(g.visible){const b=visibleBounds(g);b.translate(engineGoal(g).sub(g.position));bounds.union(b);}
  ...
  const distance=Math.max(.8,radius/Math.sin(angle)*1.13);
  const direction=name?new T.Vector3(...(presets[name]||presets.home)).normalize():camera.position.clone().sub(controls.target).normalize();
  targetGoal=centre;cameraGoal=centre.clone().addScaledVector(direction,distance);
 }
 function focus(id) {
  const g=currentGroups().get(id);if(!g)return;currentRoot().updateMatrixWorld(true);const bounds=visibleBounds(g);if(bounds.isEmpty())return;const centre=bounds.getCenter(new T.Vector3());const size=bounds.getSize(new T.Vector3()).length();
 ...
 function animate(){if(disposed)return;frame=requestAnimationFrame(animate);const now=performance.now();const dt=Math.min((now-lastTime)/1000,.5);lastTime=now;const cameraBlend=reducedMotion?1:1-Math.exp(-7*dt);const partBlend=reducedMotion?1:1-Math.exp(-10*dt);
  ...
  occlusion.enabled=!current.assembly&&current.system==='all'&&!current.isolate&&!current.hideBody&&!current.wireframe&&current.explode<.01;
 ...
 function getPartBounds(id){const g=enginePartById.has(id)?engine?.groups.get(id):groups.get(id);if(!g)return null;scene.updateMatrixWorld(true);const b=visibleBounds(g);return b.isEmpty()?null:{min:b.min.toArray(),max:b.max.toArray()};}
 return {update,view,focus,frameAssembly,resize,getCamera:()=>({position:camera.position.toArray(),target:controls.target.toArray()}),restoreCamera:s=>{cameraGoal=new T.Vector3(...s.position);targetGoal=new T.Vector3(...s.target);dirty=true;},getVisibleParts:()=>[...currentGroups()].filter(([,g])=>g.visible).map(([id])=>id),getState:()=>({...current}),getPartCount:()=>groups.size,getPartBounds,getModelStats:()=>{let meshes=0,triangles=0;currentRoot().traverseVisible(o=>{if(o.isMesh){meshes++;triangles+=(o.geometry.index?.count||o.geometry.attributes.position.count)/3;}});return{meshes,triangles};},dispose(){disposed=true;cancelAnimationFrame(frame);observer.disconnect();controls.dispose();scene.traverse(o=>{o.geometry?.dispose();if(o.material)for(const m of [o.material].flat())m.dispose();});occlusion.dispose();output.dispose();composer.dispose();envMap.dispose();pmrem.dispose();renderer.dispose();}};
}
