...
export function createEngineDetail(){
 ...
 for(const [bank,s] of [['front',-1],['rear',1]]){
  ...
  for(let c=1;c<=3;c++){
   ...
   const small=at(x-bankOffset(s),.205),big=[x,crankY+.025,s*.014];
 ...
 for(const g of altGroups.values())for(const m of [...g.children]){m.geometry.translate(.120,.7654,-1.513);m.userData.partId='eng-alternator';groups.get('eng-alternator').add(m);}
