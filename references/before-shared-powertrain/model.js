...
export function createVehicle() {
 ...
 for(const [id,g] of groups)if(g.userData.system==='engine'||['ac-compressor','thermostat'].includes(id))g.traverse(m=>{if(m.isMesh){m.updateMatrix();m.geometry.applyMatrix4(m.matrix);m.geometry.applyMatrix4(engineEnvelope);m.position.set(0,0,0);m.rotation.set(0,0,0);m.scale.set(1,1,1);}});
 ...
 for(const id of ['engine-block','heads','intake','oil-pan','ac-compressor','thermostat']){
