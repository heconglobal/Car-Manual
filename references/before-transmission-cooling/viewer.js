...
import {engineParts,enginePartById,inEngineSection} from './engine-catalog.js';
...
export function createViewer(container, onSelect) {
 ...
 function update(next) {
  ...
  root.visible=!current.assembly;floor.visible=!current.assembly;if(engine)engine.root.visible=!!current.assembly;
  ...
  for(const [id,g] of currentGroups()) {
   ...
   g.visible=current.assembly?inEngineSection(enginePartById.get(id),current.assembly):!(current.hideBody&&g.userData.system==='body'&&id!=='spaceframe');
 ...
 function fitInspectionShadow(){
  ...
  if(!current.assembly){
   ...
   engine.root.updateMatrixWorld(true);const b=new T.Box3();
   for(const g of engine.groups.values())if(g.visible){const p=visibleBounds(g);p.translate(engineGoal(g).sub(g.position));b.union(p);}
   if(b.isEmpty())return;
 ...
 function frameAssembly(name){
  ...
  const direction=name?new T.Vector3(...(name==='home'&&current.assembly==='water-pump-detail'?[-.9,.65,1.15]:name==='home'&&['coil-detail','distributor-detail'].includes(current.assembly)?[.9,.8,1.6]:presets[name]||presets.home)).normalize():camera.position.clone().sub(controls.target).normalize();
