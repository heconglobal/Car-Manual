...
export function buildHeadlamps(h,hoodPoint){
 ...
 for(const s of [-1,1]){
  ...
  const coverPoint=(u,v)=>[x+(u-.5)*.258,.711+Math.sin(.72)*.318*(1-v),-1.375-Math.cos(.72)*.318*(1-v)];
  ...
  for(const edge of [-1,1])surface(id,28,16,(u,v)=>{
   const bottom=.708+.006*u,top=.906*(1-u)+.714*u;
   return [x+edge*(.120-.005*u+.002*Math.sin(v*Math.PI)),bottom+(top-bottom)*v,z+.244*u];
  },'plastic',up);
  surface(id,24,8,(u,v)=>[x+(u-.5)*.235,.711,z+.242*v],'plastic',up);
  ...
  tube(id,[[mx,.580,-1.415],[x-s*.112,.680,-1.49],[x-s*.112,.768,-1.515]],.006,'metal',up);
