...
function collect(value){
 if(typeof value==='string'&&/^https?:\/\//.test(value)){
  const u=new URL(value),fragment=u.hash;u.hash='';
  if(!urls.has(u.href))urls.set(u.href,new Set());if(fragment)urls.get(u.href).add(fragment);
 }else if(Array.isArray(value))value.forEach(collect);
 else if(value&&typeof value==='object')Object.values(value).forEach(collect);
}
collect([sources,parts,tours,detailParts]);
const report={results:[...urls].map(([url,fragments])=>({url,fragments:[...fragments]}))};
...
const counts=JSON.parse((await promisify(execFile)('python3',['-c',`import fitz,json,sys,hashlib,pathlib
r={}
...
print(json.dumps(r))`,JSON.stringify(map.map(([,file])=>file))],{encoding:'utf8'})).stdout);
const results=[];
...
const result={date:new Date().toISOString(),sourceSha256:sourceFingerprint(),status:results.every(r=>r.valid)?'passed':'failed',pageLinks:results.length,documents:counts,results,unmappedDocuments:report.results.filter(r=>!map.some(([key])=>r.url.includes(key))).map(r=>r.url),limits:'Collects current catalog and guide references directly, without relying on a historical network report. Confirms that linked PDF page numbers exist in the downloaded reference edition; does not verify the content of each cited page, unmapped documents, remote reachability or original-vehicle applicability.'};
