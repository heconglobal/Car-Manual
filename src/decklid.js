...
export function buildDecklid(h,deckHeight){
 ...
 const lidPoint=(x,z)=>{
  const hump=.058*T.MathUtils.smoothstep(z,.765,.87)*(1-T.MathUtils.smoothstep(z,1.015,1.34));
 ...
 for(const s of [-1,1]){
  ...
  tube('decklid',Array.from({length:32},(_,i)=>lidPoint(s*.650,lerp(1.185,1.861,i/31))),.0022,'rubber');
