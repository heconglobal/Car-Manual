...
export function buildFascias(h){
 ...
 for(const rear of [false,true]){
  ...
  const mesh=surface(id,112,64,(u,v)=>{const x=(u*2-1)*width(v,rear),y=lerp(.245,top+crown(x,rear?.650:.659,rear),v);return point(x,y,rear);});
  ...
  for(let i=0;i<idx.count;i+=3){const a=idx.getX(i),b=idx.getX(i+1),c=idx.getX(i+2),x=Math.abs((pos.getX(a)+pos.getX(b)+pos.getX(c))/3),y=(pos.getY(a)+pos.getY(b)+pos.getY(c))/3;const lamp=rear?(x>.032&&x<.722&&y>.622&&y<.718):(Math.abs(x-.557)<.051&&Math.abs(y-.365)<.019);if(!lamp)keep.push(a,b,c);}
 ...
 function patch(id,face,cx,cy,w,h,offset,mat){
  const mesh=surface(id,96,14,(u,v)=>{
  ...
  if(id==='nose'&&mat==='rubber'){
   const g=mesh.geometry,p=g.attributes.position,ix=g.index,keep=[];
   for(let i=0;i<ix.count;i+=3){const indices=[ix.getX(i),ix.getX(i+1),ix.getX(i+2)],x=Math.abs(indices.reduce((n,k)=>n+p.getX(k),0)/3),y=indices.reduce((n,k)=>n+p.getY(k),0)/3;if(!(Math.abs(x-.557)<.053&&Math.abs(y-.365)<.021))keep.push(...indices);}
 ...
 // The shared lighting builder owns the complete rear lamp assemblies.
}
