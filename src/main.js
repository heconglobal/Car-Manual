...
function saveConfiguration(){const previousScope=state.assembly,hiddenSelection=state.assembly&&!detailAvailable(detailPartById.get(state.selected),state.configuration);if(hiddenSelection){state.selected=null;state.isolate=false;}if(state.assembly&&!detailMembers(state.assembly).some(p=>detailAvailable(p,state.configuration))){state.assembly=familyFor(state.assembly).id;state.selected=null;state.isolate=false;state.assemblyExplode=0;}renderSystems();renderParts();try{localStorage.setItem('fiero-configuration-v2',JSON.stringify(serviceReturn?{...state.configuration,headlights:serviceReturn.state.configuration.headlights}:state.configuration));}catch{toast('Preview changed; browser storage is unavailable.');}sync();if(state.assembly&&(previousScope!==state.assembly||hiddenSelection))viewer?.frameAssembly('home');}
...
function finishTour(){
 if(serviceReturn){
  const prior=serviceReturn;serviceReturn=null;
  const configuration={...state.configuration,headlights:prior.state.configuration.headlights};
...
function applyStep(index){
 ...
 if(tour.kind==='service'){
  if(!serviceReturn){
   serviceReturn={state:{...state,tour:null,configuration:{...state.configuration}},vehicleReturn,camera:viewer?.getCamera()};
  ...
  Object.assign(state,{assembly:tour.assembly,assemblyExplode:0,system:tour.system,selected:step.part,query:'',isolate:false,hideBody:false,wireframe:false,step:stepIndex,tab:'tours',camera:'home',configuration:{...state.configuration,headlights:true}});
