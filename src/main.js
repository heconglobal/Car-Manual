import {remainingWork} from './remaining-work.js';
import {coverageAudit} from './coverage.js';
import './style.css';
import { vehicle, systems, parts, tours, sources, acceptance } from './data.js';
import { createViewer } from './viewer.js';
import {engineParts,engineSource} from './engine-catalog.js';
import {detailParts,detailPartById,detailSections,detailSectionById,detailMembers,familyFor,detailFamilies,detailAvailable} from './inspection-catalog.js';
...
document.querySelector('#app').innerHTML=`
 <header class="header">
  ...
  <div class="header-actions"><span class="build-badge"><i></i> REVIEW 0.2 · BODY R8</span><button class="quiet" data-action="configure">${icon('gear')}<span>Configure</span></button><button class="quiet" data-action="sources">${icon('book')}<span>Reference library</span></button></div>
 </header>
 ...
 <main class="workspace">
  ...
  <section class="stage" aria-label="3D vehicle explorer">
   <div class="stage-heading"><div><p class="eyebrow" id="stage-kicker">01 / COMPLETE VEHICLE</p><h2 id="stage-title">Anatomy of a Fiero<span>.</span></h2><p id="stage-subtitle">Left = driver · Right = passenger · viewed facing forward.</p></div><div class="stage-badges"><span id="drive-layout">US LEFT-HAND DRIVE</span><span class="outline-badge">REFERENCE RECONSTRUCTION</span></div></div>
   ...
   <div class="camera-views" role="group" aria-label="Camera views">${['home','front','rear','side','passenger','top'].map((v,i)=>`<button data-view="${v}" aria-pressed="${i===0}">${v==='home'?'Perspective':v==='side'?'Driver side':v==='passenger'?'Passenger side':v[0].toUpperCase()+v.slice(1)}</button>`).join('')}</div>
...
function getPart(){return state.assembly?detailPartById.get(state.selected):parts.find(p=>p.id===state.selected);}
function systemName(id){return systems.find(s=>s.id===id)?.name||'Whole vehicle';}
function renderSystems(){
 const family=familyFor(state.assembly);
 const sections=detailSections.filter(s=>(!s.parent||s.family===family.id&&(s.parent===family.id||s.parent===state.assembly||s.parent===detailSectionById.get(state.assembly)?.parent))&&detailMembers(s.id).some(p=>detailAvailable(p,state.configuration)));
 document.querySelector('#system-count').textContent=String(state.assembly?sections.length:systems.length-1).padStart(2,'0');
 if(state.assembly){document.querySelector('#systems').innerHTML=sections.map(s=>`<button class="system-button ${state.assembly===s.id?'active':''} ${s.parent&&s.parent!==family.id?'nested-system':''}" data-assembly="${s.id}" aria-pressed="${state.assembly===s.id}">${icon(detailFamilies[s.family].icon)}<span>${s.name}</span><small>${detailMembers(s.id).filter(p=>detailAvailable(p,state.configuration)).length}</small></button>`).join('');return;}
 document.querySelector('#systems').innerHTML=systems.map(s=>`<button class="system-button ${state.system===s.id?'active':''}" data-system="${s.id}" aria-pressed="${state.system===s.id}">${icon(s.icon)}<span>${s.name}</span><small>${s.id==='all'?parts.length:parts.filter(p=>p.system===s.id).length}</small></button>`).join('');
}
function renderParts(){
 ...
 const candidates=state.assembly?detailMembers(state.assembly):q?[...parts,...detailParts]:parts.filter(p=>state.system==='all'||p.system===state.system);
 const filtered=candidates.filter(p=>detailAvailable(p,state.configuration)&&(!q||matches(p)));
 ...
 document.querySelector('#part-list').innerHTML=filtered.length?filtered.map(p=>`<button class="part-button ${state.selected===p.id?'active':''}" data-part="${p.id}" aria-pressed="${state.selected===p.id}"><span class="part-dot" style="--part-color:${systems.find(s=>s.id===p.system).color}"></span><span>${p.name}${!state.assembly&&detailPartById.has(p.id)?`<small class="part-scope">${detailFamilies[p.family].name} / ${detailSectionById.get(p.section).name}</small>`:''}</span>${icon('chevron')}</button>`).join(''):`<div class="empty-state">No components match “${esc(state.query)}”.<br/><button class="text-button" data-action="clear-search">Clear search</button></div>`;
}
...
function saveConfiguration(){const previousScope=state.assembly,hiddenSelection=!detailAvailable(detailPartById.get(state.selected)||parts.find(p=>p.id===state.selected),state.configuration);if(hiddenSelection){state.selected=null;state.isolate=false;}if(state.assembly&&!detailMembers(state.assembly).some(p=>detailAvailable(p,state.configuration))){state.assembly=familyFor(state.assembly).id;state.selected=null;state.isolate=false;state.assemblyExplode=0;}renderSystems();renderParts();try{localStorage.setItem('fiero-configuration-v2',JSON.stringify(configurationWithoutGuidePose()));}catch{toast('Preview changed; browser storage is unavailable.');}sync();if(state.assembly&&(previousScope!==state.assembly||hiddenSelection))viewer?.frameAssembly('home');}
function renderInspector(){
 ...
 if(state.tab==='component')panel.innerHTML=part?`
  ...
  ${notice()}<div class="coverage"><span>UAT scope</span><strong>${parts.length} assemblies · ${tours.length} guided tours</strong><small>Interaction prototype. Full repair-manual content is still in development.</small></div>`;
 if(state.tab==='tours') {
  const tour=tours.find(t=>t.id===state.tour);
  panel.innerHTML=tour?`<button class="text-button back-button" data-action="exit-tour">← All guides</button><p class="eyebrow">${tour.kind==='service'?'FACTORY PROCEDURE / 3D PART REFERENCES':'3D ASSEMBLY WALKTHROUGH'}</p><h2>${tour.title}</h2>${tour.tools?`<p class="description small"><strong>Tools:</strong> ${tour.tools}</p>`:''}<div class="tour-progress">${tour.steps.map((s,i)=>`<button data-step="${i}" class="${state.step===i?'current':state.step>i?'done':''}" aria-label="Step ${i+1}: ${s.title}" aria-current="${state.step===i?'step':'false'}">${i+1}</button>`).join('')}</div><p class="eyebrow">STEP ${state.step+1} OF ${tour.steps.length}</p><h3 class="step-title">${tour.steps[state.step].title}</h3><p class="description step-description">${tour.steps[state.step].text}</p>${tour.kind==='service'?`<button class="secondary full-width" data-action="inspect-guide-part">${state.isolate?'Show assembly':'Inspect this part'}</button>`:''}<div class="tour-nav"><button class="secondary" data-action="prev-step" ${state.step===0?'disabled':''}>← Previous</button><button class="primary" data-action="next-step">${state.step===tour.steps.length-1?'Finish tour':'Next step'} ${icon('arrow')}</button></div>${tour.kind==='service'?`${notice(tour.caution)}<p class="description small"><a href="${tour.steps[state.step].sourceUrl}" target="_blank" rel="noopener noreferrer">${tour.source} ↗</a><br/>Sequence checked against the factory text; illustrated paths and local dimensions remain reconstructed.</p>`:notice('Assembly visualization only. These steps are not a validated repair procedure.')}<p class="section-label">IN THIS WALKTHROUGH</p><ol class="step-list">${tour.steps.map((s,i)=>`<li><button data-step="${i}" class="${state.step===i?'active':''}"><span>${state.step>i?'✓':String(i+1).padStart(2,'0')}</span>${s.title}</button></li>`).join('')}</ol>`:
   `<p class="eyebrow">LEARN BY EXPLORING</p><h2>Inside every assembly.</h2><p class="description">Guided 3D tours highlight components and move the camera as you explore.</p>${tours.map((t,i)=>`<button class="tour-card" data-tour="${t.id}"><span class="tour-card-top"><span>0${i+1} / ${t.kind==='service'?'FACTORY PROCEDURE':t.id==='air-cleaner'?'ASSEMBLY DEMO':'SYSTEM TOUR'}</span>${icon(i===0?'engine':i===1?'layers':'spring')}</span><strong>${t.title}</strong><span class="tour-card-bottom">${t.subtitle}${icon('arrow')}</span></button>`).join('')}${notice('The headlamp and coolant guides follow the original 1985 factory sequences. The other guides explain assembly relationships. Model movement does not establish real removal clearance.')}`;
 }
...
function engineInspector(part){
 const section=detailSectionById.get(state.assembly),family=familyFor(state.assembly),children=detailSections.filter(s=>s.parent===state.assembly&&detailMembers(s.id).some(p=>detailAvailable(p,state.configuration)));
 return `<nav class="assembly-breadcrumb" aria-label="Assembly path"><button class="text-button" data-action="exit-assembly">Vehicle</button><span>/</span><button class="text-button" data-assembly="${family.id}">${family.shortName}</button>${section.parent&&section.parent!==family.id?`<span>/</span><button class="text-button" data-assembly="${section.parent}">${detailSectionById.get(section.parent).name}</button>`:''}${state.assembly!==family.id?`<span>/</span><span>${section.name}</span>`:''}</nav>
 <p class="eyebrow">${family.name.toUpperCase()} COMPONENT EXPLORER</p><div class="component-heading"><h2>${part?part.name:section.name}</h2></div>
 <p class="description">${part?part.description:state.assembly===family.id?'Separate the assembly to inspect its internal components. Open a subassembly below for a closer breakdown.':'Use the explode slider to separate this subassembly. Select a component in the model or list to inspect it.'}</p>
 ...
 ${part?`<div class="detail-actions"><button class="secondary" data-action="focus">Focus part</button><button class="secondary" data-action="isolate" aria-pressed="${state.isolate}">${state.isolate?'Show context':'Isolate'}</button></div><p class="description small">${part.source} · <a href="${part.sourceUrl||`${engineSource}#page=${part.source.endsWith('H-19')?14:17}`}" target="_blank" rel="noopener noreferrer">View reference ↗</a>${part.callout?` · callout ${part.callout}`:''}</p>${part.referenceNote?`<p class="description small">${part.referenceNote}</p>`:''}${part.serviceReference?`<section class="spec-group part-service-reference"><h3>${esc(part.serviceReference.title)}</h3>${part.serviceReference.rows.map(([key,value])=>`<div class="spec-row"><span>${esc(key)}</span><strong>${esc(value)}</strong></div>`).join('')}<p class="description small">${esc(part.serviceReference.note)}</p>${part.serviceReference.links.map(([label,url])=>`<p class="description small"><a href="${url}" target="_blank" rel="noopener noreferrer">${esc(label)} ↗</a></p>`).join('')}</section>`:''}${part.relatedAssembly?`<button class="secondary full-width" data-assembly="${part.relatedAssembly}">${esc(part.relatedAssemblyLabel||'Inspect components')}</button>`:''}${part.section!==state.assembly?`<button class="secondary full-width" data-assembly="${part.section}">Open ${detailSectionById.get(part.section).name}</button>`:''}<button class="text-button reference-link" data-action="note-part">${icon('note')} Add a UAT note about this part</button>`:''}
 ${children.length?`<p class="section-label">OPEN A SUBASSEMBLY</p><div class="engine-section-list">${children.map(s=>`<button class="guide-link" data-assembly="${s.id}"><span>${s.name}<small>${detailMembers(s.id).filter(p=>detailAvailable(p,state.configuration)).length} selectable parts / sets</small></span>${icon('chevron')}</button>`).join('')}</div>`:`<button class="text-button reference-link" data-assembly="${family.id}">← ${detailSectionById.get(family.id).name}</button>`}
 ${family.id==='headlight-system'?`<div class="detail-actions"><button class="secondary" data-action="headlight-pose" aria-pressed="${state.configuration.headlights}">${state.configuration.headlights?'Lower headlights':'Raise headlights'}</button><span class="description small">${state.configuration.headlights?'Raised':'Closed'} endpoint pose</span></div>`:''}
 ${family.id==='hvac-system'?`<p class="description small">Viewing ${state.configuration.airConditioning?'C60 air conditioning':'C41 standard heater'}. <button class="text-button" data-action="configure">Change configuration</button></p>`:''}
 ${family.id==='cooling-system'?`<p class="section-label">ENGINE COOLING COMPONENTS</p><div class="engine-section-list"><button class="guide-link" data-assembly="thermostat-detail"><span>Thermostat & filler neck</span>${icon('chevron')}</button><button class="guide-link" data-assembly="water-pump-detail"><span>Water pump & pulley</span>${icon('chevron')}</button></div>`:''}
 <details class="engine-coverage"><summary>Modeled components & reference limits</summary><p>${detailMembers(family.id).length} selectable parts and grouped sets. ${family.coverage}</p><p>Grouped hardware and overlapping vehicle / detail views are not a complete parts inventory. No disassembly order, gear manufacturing data or unverified torque values are implied.</p><button class="text-button" data-action="sources">View vehicle-wide parts coverage</button></details>`;

}
function openAssembly(id){
 if(!detailSectionById.has(id))return;
 if(!state.assembly)vehicleReturn={state:{...state},camera:viewer?.getCamera()};
 Object.assign(state,{assembly:id,assemblyExplode:.65,selected:null,query:'',isolate:false,hideBody:false,tour:null,system:familyFor(id).system,tab:'component',camera:'home'});
...
function sync(){
 ...
 document.querySelector('#stage-title').innerHTML=state.system==='all'?'Anatomy of a Fiero<span>.</span>':`${system.name}<span>.</span>`;document.querySelector('#stage-subtitle').textContent=state.system==='all'?'Left = driver · Right = passenger · viewed facing forward.':system.subtitle+' · select a component to inspect';
 ...
 document.querySelector('#drive-layout').textContent=state.assembly?'FACTORY-NEW FINISH':'US LEFT-HAND DRIVE';
 ...
 document.querySelector('label[for=explode]').textContent=state.assembly?`${familyFor(state.assembly).name.toUpperCase()} EXPLODED VIEW`:'EXPLODED VIEW';
 document.querySelector('#explode').setAttribute('aria-label',state.assembly?`${familyFor(state.assembly).name} exploded view`:'Exploded view');
 ...
 document.querySelector('.sidebar-heading h1').textContent=state.assembly?`${familyFor(state.assembly).name} assemblies`:'Assembly explorer';
 ...
 if(state.assembly){document.querySelector('#stage-kicker').textContent=familyFor(state.assembly).kicker;document.querySelector('#stage-title').textContent=detailSectionById.get(state.assembly).name;document.querySelector('#stage-subtitle').textContent='Select a component · separate the assembly · look inside';document.querySelector('#selection-count').textContent=`${detailMembers(state.assembly).filter(p=>detailAvailable(p,state.configuration)).length} parts / sets`;}
}
function selectPart(id){if(serviceReturn)finishTour();if(detailPartById.has(id)&&(!state.assembly||!detailMembers(state.assembly).some(p=>p.id===id))){const part=detailPartById.get(id);openAssembly(part.section);state.selected=id;sync();viewer?.focus(id);return;}if(state.assembly){const part=detailPartById.get(id);if(!part)return;state.selected=id;state.tab='component';sync();document.querySelector('.sidebar').classList.remove('mobile-open');return;}const part=parts.find(p=>p.id===id);if(!part)return;state.selected=id;state.system=part.system;state.tab='component';state.tour=null;if(part.system==='body')state.hideBody=false;sync();viewer?.focus(id);document.querySelector('.sidebar').classList.remove('mobile-open');}
function configurationWithoutGuidePose(){
 const result={...state.configuration};
 for(const key of serviceReturn?.temporaryKeys||[])result[key]=serviceReturn.state.configuration[key];
 return result;
}
function finishTour(){
 if(serviceReturn){
  const prior=serviceReturn,configuration=configurationWithoutGuidePose();serviceReturn=null;
  Object.assign(state,prior.state,{configuration,tour:null,tab:'tours'});vehicleReturn=prior.vehicleReturn;
  document.querySelector('#search').value=state.query;sync();if(prior.camera)viewer?.restoreCamera(prior.camera);
 }else{state.tour=null;state.explode=0;sync();}
}
function applyStep(index){
 const tour=tours.find(t=>t.id===state.tour);if(!tour)return;
 if(tour.kind==='service'){
  if(!serviceReturn){
   serviceReturn={state:{...state,tour:null,configuration:{...state.configuration}},temporaryKeys:Object.keys(tour.configuration||{}),vehicleReturn,camera:viewer?.getCamera()};
   if(!state.assembly)vehicleReturn={state:{...state,tour:null},camera:viewer?.getCamera()};
  }
  const stepIndex=Math.max(0,Math.min(index,tour.steps.length-1)),step=tour.steps[stepIndex];
  Object.assign(state,{assembly:step.assembly||tour.assembly,assemblyExplode:0,system:step.system||tour.system,selected:step.part,query:'',isolate:false,hideBody:false,wireframe:false,step:stepIndex,tab:'tours',camera:'home',configuration:{...state.configuration,...tour.configuration}});
  document.querySelector('#search').value='';sync();viewer?.frameAssembly('home');return;
 }
 if(state.assembly){const tourId=state.tour;exitAssembly();state.tour=tourId;}
 state.step=Math.max(0,Math.min(index,tour.steps.length-1));const step=tour.steps[state.step];state.system=step.system;state.selected=step.part||null;state.isolate=false;state.hideBody=false;state.explode=step.explode||0;state.camera=step.camera;state.tab='tours';sync();viewer?.view(step.camera);
 if(tour.id==='air-cleaner')setTimeout(()=>{if(state.tour===tour.id&&state.step===index)viewer?.focus(step.part);},180);
}
...
function openModal(type){
 ...
 if(type==='remaining-work'){document.querySelector('#modal-content').innerHTML=`<div class="modal-heading"><p class="eyebrow">DEVELOPMENT CHECKLIST</p><button class="icon-button" data-action="close-modal" aria-label="Close dialog">${icon('close')}</button></div><h2>What remains to build.</h2><p class="description">Headlights first. This tracks known gaps in the 3D model and manual; it is not a list of repairs your car needs. A modeled part can still need measurement and verification.</p>${remainingWork.map((row,i)=>`<article class="source-card"><h3>${i+1}. ${esc(row.name)}</h3><p class="eyebrow">${esc(row.stage)}</p><ul>${row.items.map(t=>`<li>${esc(t)}</li>`).join('')}</ul>${row.assembly?`<button class="text-button" data-work-assembly="${row.assembly}">Inspect current model ${icon('arrow')}</button>`:''}</article>`).join('')}`;modal.showModal();return;}

 document.querySelector('#modal-content').innerHTML=`<div class="modal-heading"><p class="eyebrow">WORKSHOP / 85</p><button class="icon-button" data-action="close-modal" aria-label="Close dialog">${icon('close')}</button></div>${type==='sources'?`<h2>Reference library</h2><button class="secondary full-width" data-action="remaining-work">Remaining model & manual work</button><p class="description">Source coverage is explicit. A source listing does not mean all of its content has been verified or imported.</p><details class="coverage-audit"><summary>Parts coverage · modeled and remaining</summary><p class="description small">${parts.length} vehicle assemblies and ${detailParts.length} detailed parts / grouped sets across the component explorers. Counts overlap where the same part appears in both views; they are not a complete vehicle inventory.</p>${coverageAudit.map(row=>`<article class="coverage-row"><h3>${row.system}</h3><p><strong>Modeled:</strong> ${row.modeled}</p><p><strong>Still missing or simplified:</strong> ${row.remaining}</p></article>`).join('')}</details>${sources.map(s=>`<article class="source-card"><h3>${s.title}</h3><p>${s.note}</p>${s.url?`<a href="${s.url}" target="_blank" rel="noopener noreferrer">Open source ↗</a>`:''}</article>`).join('')}`:`<h2>A 3D workshop in progress.</h2><p class="description">Body R8 refines the entire exterior: panel and molding joins, quarter openings, roof-pillar fit, bumpers, lamps, wheels and surface finish. Sixteen selected body dimensions are checked; two bumper ground-reference interpretations and exact local tooling remain unresolved. Build 0.2 includes: ${parts.length} selectable assemblies, nine systems, ${tours.length} guided tours, exploded views and local UAT feedback.</p><div class="source-card"><h3>What this build represents</h3><p>The vehicle is an owner-described original 1985 Fiero SE 2M6, four-speed manual, with WS6. VIN identity was checked with NHTSA.</p></div><div class="source-card"><h3>What is still in development</h3><p>The procedural model is illustrative, not a measured replica. The engine now has selectable internal parts and a detailed ignition breakdown. Many fasteners, measured gear profiles, exact routing, full accessory options and validated repair instructions remain incomplete. Technical service values are withheld until checked against factory references.</p></div><div class="source-card"><h3>Your data</h3><p>Feedback and checklist results are stored locally in this browser. Export them from the UAT tab to share or back up. No analytics, accounts or external assets are loaded by the app.</p></div>`}`;modal.showModal();
}
document.addEventListener('click',async e=>{
 const b=e.target.closest('button');if(!b)return;
 if(serviceReturn&&(b.dataset.part||b.dataset.system||b.dataset.assembly||b.dataset.workAssembly||b.dataset.tour&&b.dataset.tour!==state.tour||['reset','exit-assembly','show-exterior','show-cabin','show-chassis'].includes(b.dataset.action)))finishTour();
 if(b.dataset.workAssembly){document.querySelector('#modal').close();openAssembly(b.dataset.workAssembly);return;}
 ...
 switch(b.dataset.action){
  case 'headlight-pose':state.configuration.headlights=!state.configuration.headlights;saveConfiguration();viewer?.frameAssembly();break;
  ...
  case 'show-cabin':if(state.assembly)exitAssembly();state.system='interior';state.selected=null;state.hideBody=true;state.isolate=false;state.camera='cabin';sync();viewer?.view('cabin');break;
  ...
  case 'remaining-work':openModal('remaining-work');break;
  ...
  case 'inspect-guide-part':state.isolate=!state.isolate;sync();if(state.isolate)viewer?.focus(state.selected);else viewer?.frameAssembly('home');break;
  case 'prev-step':applyStep(state.step-1);break;
  case 'next-step':{const tour=tours.find(t=>t.id===state.tour);if(state.step===tour.steps.length-1){finishTour();toast('Guide complete — explore another assembly');}else applyStep(state.step+1);break;}
  case 'exit-tour':finishTour();break;
  case 'export':{const payload={build:'0.2.0',bodyRevision:'R8',configuration:state.configuration,exportedAt:new Date().toISOString(),vehicle,checkedItems:saved.checks.map(i=>acceptance[i]),notes:saved.notes};const url=URL.createObjectURL(new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download='fiero-uat-feedback.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);toast('UAT feedback exported');break;}
...
document.querySelector('.brand').addEventListener('click',e=>{e.preventDefault();if(serviceReturn)finishTour();if(state.assembly)exitAssembly();state.tab='component';reset();});
